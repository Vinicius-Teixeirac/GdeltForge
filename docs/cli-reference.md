...
| `--recover-unzipped` | Convert leftover CSVs sitting in `unzipped_data_directory` instead of the ZIPs in `downloaded_data_directory`. Off by default; not supported for `--dataset events-reduced` |
...
`--recover-unzipped` is for the CSV a normal run leaves behind: the extraction step succeeded but the parquet write itself failed, or `converter.keep_unzipped` kept the CSV around after a config change invalidated its `.done` marker, and by the time someone wants to retry, the source ZIP is gone (`--delete-source` already removed it, or it was cleaned up by hand). A plain rerun of `convert` only looks at `downloaded_data_directory`, so it can't see that CSV at all; this flag points the same conversion logic at `unzipped_data_directory` instead, with its own `.done` tracking keyed on the CSV rather than the ZIP. It never runs as part of a normal `convert`; call it deliberately once you know a ZIP is missing. Not supported for `--dataset events-reduced`, which streams and partitions its single source file directly rather than leaving a per-file CSV behind.
