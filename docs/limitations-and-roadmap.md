...
- [x] `convert`'s own parquet writes were left hardcoded to `snappy` when `filter.compression` defaulted to `zstd` (see above); there was no `converter.compression` at all, so converter's intermediate output couldn't benefit from the same measured savings and had no setting to include in the fingerprint fix above either. `converter.compression` now exists, same per-dataset shape as `filter.compression`, defaulting to `zstd` and included in the `.done` marker fingerprint
