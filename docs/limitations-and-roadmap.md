...
- [x] `convert` had no resumability for flat/daily output (Events daily, GKG 1.0/2.1, Mentions), unlike `scrape`'s skip-already-downloaded behavior: a `.done` marker file already existed for historical (Hive-partitioned yearly/monthly) output, but was never applied to the flat case. Found for real against a live 30,137-file Mentions batch: two independent runs each died to an OS-level kill around the same ~51% mark after 30+ minutes, with zero parquet output ever finalized, because every relaunch reprocessed every zip from file 1. The marker is now written for every file type, and `process_all_files` skips any zip already marked done on the next run, matching `scrape`'s resume behavior
- [x] `crossref_events_gkg_auto`'s original DATEADDED-based routing (see above) excluded an event from `v2` whenever its own `DATEADDED` predated `GKG_V2_COVERAGE_START`, on the reasoning that such an event could only ever match through GKG 1.0. Confirmed for real that reasoning doesn't hold, on two levels: a Mentions row is timestamped by when it was created, not by its event's `DATEADDED` (a 2019-origin event was found referenced by a real Mentions row dated 2020), and tested directly against the complete 2015-02-18 through 2015-12-31 Mentions history (211.8M rows) plus a further scattered sample, finding zero rows referencing any of the 186.5M events with a pre-2015-02-18 `DATEADDED` -- not because none could exist, but because the routing itself never gave any of them the chance to be found. Every eligible event (`DATEADDED >= GKG_V1_COVERAGE_START`) is now attempted against both generations rather than routed to exactly one; an event matching both contributes one row per source instead of the routing logic picking (or missing) one. The same asymmetry existed in the other direction too (GKG 1.0 remains live today, so a recent event isn't guaranteed to be GKG-2.1-only), fixed the same way
